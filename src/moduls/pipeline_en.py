...
regex_stopwords = [
    r'\brobot\w{0,3}\b',   # Match words starting with "robot"
]
# Compile regex patterns
stopword_patterns = [re.compile(pattern) for pattern in regex_stopwords]

pipe_cfg = PipeConfig()


def is_stopword(word):
        # Check if token matches any regex stop word patterns
    for pattern in stopword_patterns:
        if pattern.search(word):
            return True
    return word in custom_stop_words
